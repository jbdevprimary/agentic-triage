...
async function fetchAllIssues(): Promise<IssueMetrics[]> {
    ...
    // Search for open issues via GitHub MCP
    const issues = await searchIssues('is:open is:issue');
    ...
    return issues.map((issue) => ({
        number: issue.number,
        title: issue.title,
        nodeId: `issue_${owner}_${repo}_${issue.number}`,
        createdAt: now,
        updatedAt: now,
        closedAt: undefined,
        reactions: 0,
...
async function createGitHubMilestone(
    ...
    // Return a fake milestone number - actual milestone creation requires
    // either the GitHub REST API or using runAgenticTask with extended MCP tools
    return 0;
}

async function assignToMilestone(issueNumber: number, milestoneNumber: number): Promise<void> {
    if (milestoneNumber === 0) {
        // Skip if milestone wasn't created
        return;
    }

    // Milestone assignment via MCP would need updateIssue with milestone support
    // For now, just log the intention
    console.log(`Would assign #${issueNumber} to milestone ${milestoneNumber}`);
}
