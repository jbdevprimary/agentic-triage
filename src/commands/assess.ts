...
import { addIssueComment, addIssueLabels, getIssue, getRepoContext } from '../octokit.js';
...
**Current Labels:** ${issue.labels.join(', ') || 'none'}`;
    ...
    if (typeMatch) {
        ...
        if (validLabels.includes(typeLabel) && !issue.labels.includes(typeLabel)) {
            ...
            await addIssueLabels(issueNumber, [typeLabel]);
        }
    }
    ...
    if (response.includes('<!-- ready-for-aider -->')) {
        ...
        await addIssueLabels(issueNumber, ['ready-for-aider']);
    }
