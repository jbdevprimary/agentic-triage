...
import { addIssueComment, commentOnPR, getIssue, getPullRequest } from '../octokit.js';
...
export async function test(options: TestOptions = {}): Promise<void> {
    ...
    if (issue) {
        ...
        const prData = await getPullRequest(pr);
        title = prData.title;
        body = prData.body;
        // Note: our MCP wrapper doesn't currently fetch full diffs; keep empty here.
        diff = '';
    ...
    try {
        ...
        // Comment on issue/PR
        const summary = result.text.length > 1000 ? `${result.text.slice(0, 1000)}...` : result.text;

        const comment = `## 🧪 AI Test Generation\n\n${summary}\n\n---\n_Generated by @strata/triage_`;

        if (issue) {
            await addIssueComment(issue, comment);
        } else if (pr) {
