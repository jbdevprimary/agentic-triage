...
export async function verify(prNumber: number, options: VerifyOptions = {}): Promise<void> {
    ...
    try {
        ...
        console.log(pc.green('✅ Unit tests passed'));
    } catch (err) {
        unitTestResult = (err as { stdout?: string }).stdout || 'Tests failed';
        console.log(pc.yellow('⚠️ Some unit tests failed'));
    }
    ...
    try {
        ...
        console.log(pc.yellow('⚠️ E2E tests failed or not available'));
    }
...
Take a screenshot and check for any visual issues or console errors.`;
                ...
                console.log(pc.green('✅ Browser verification complete'));
            } catch (err) {
                browserVerification = `Browser verification failed: ${err instanceof Error ? err.message : err}`;
                console.log(pc.yellow('⚠️ Browser verification failed'));
            } finally {
    ...
    const report = `## 🔍 Verification Report for PR #${prNumber}
...
_Generated by @strata/triage_`;
    ...
    await commentOnPR(prNumber, report);
